...
interface ClimbDetailProps {
  /** API base for the climb detail, e.g. "/api/climbs" or "/api/archive-climbs". */
  endpoint: string;
  /** Base path for back-link and share URL, e.g. "/climbs" or "/archive". */
  backHref: string;
  uuid: string;
}

export function ClimbDetail({ endpoint, backHref, uuid }: ClimbDetailProps) {
  ...
  const handleShare = async () => {
    if (!climb) return;
    const url = `${window.location.origin}${backHref}/${climb.climb_uuid}`;
    if (navigator.share) {
      navigator.share({
        title: climb.name,
