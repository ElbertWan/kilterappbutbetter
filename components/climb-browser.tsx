...
interface ClimbBrowserProps {
  /** API endpoint returning { climbs, total, hasMore }, e.g. "/api/climbs". */
  endpoint: string;
  /** Base path for climb detail links, e.g. "/climbs" or "/archive". */
  hrefBase: string;
  /** Heading shown in the sticky header. */
  title: string;
  /** localStorage key for persisting filter-panel selections. */
  storageKey: string;
  /** Optional note rendered under the search bar (e.g. archive explainer). */
  banner?: ReactNode;
}
      ...
      <div className="mx-auto max-w-2xl px-4 py-2">
        {isLoading && page === 0 ? (
          ...
          <>
            <div>
              {climbs.map((climb) => (
                <ClimbCard key={climb.climb_uuid} climb={climb} hrefBase={hrefBase} />
              ))}
            </div>
